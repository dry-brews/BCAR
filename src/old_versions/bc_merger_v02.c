...
// Global variables to track error rates
double call_correct = 0.0;
...
int main(int argc, char *argv[]) {
    ...
    fprintf(stderr, "%% Indels: %.4f%%\n", (call_indel / call_correct) * 100.0);
    fprintf(stderr, "%% Missense: %.4f%%\n", (call_missense / call_correct) * 100.0);
    ...
    for (int y = 1; y < ylen; y++) {
        for (int x = 1; x < xlen; x++) {
            ...
            if (diag >= gap_x.data[y][x] && diag >= gap_y.data[y][x]) {
                ...
                trace.data[y][x] = (open_x > extend_x) ? 2 : 4; // 2 for gap open, 4 for gap extend in query
            } else {
                score.data[y][x] = gap_y.data[y][x];
                trace.data[y][x] = (open_y > extend_y) ? 3 : 5; // 3 for gap open, 5 for gap extend in reference
            }
        }
    }
    ...
    while (x > 0 && y > 0) {
        ...
        switch(trace_val) {
            case 1:  // Match/mismatch
                ...
                y--;
                break;
            case 2:  // Gap open in query
            case 4:  // Gap extend in query
                x--;
                break;
            case 3:  // Gap open in reference
            case 5:  // Gap extend in reference
                y--;
                break;
        }
    }
...
SeqArray merge_seqs(const SeqArray* seq1, const SeqArray* seq2) {
    ...
    for(int i=0; i<trace_len; i++) {
        Position p1 = {0}, p2 = {0};  // Initialize to zero
        
        // Handle sequence boundaries safely
        if (pos1 < seq1->length) {
        ...
        switch(trace[i]) {
            ...
            case 4:  // gap extend in query
                ...
                merged.positions[i].scores[4] = 0;  // Reset gap score before accumulating
                for(int j=0; j<5; j++) {
...
void array_to_seq(const SeqArray* array, char** seq_out, char** qual_out) {
    ...
    *qual_out = malloc(array->length + 1);
    ...
    for(int i = 0; i < array->length; i++) {
        ...
        // Update error rate variables
        call_correct += max_val; // Add confidence for the consensus base
        call_indel += pos->scores[4]; // Add confidence for the gap character
        for (int j = 0; j < 4; j++) { // Add confidence for non-consensus, non-gap bases
            if (j != max_idx) {
    ...
    if(out_pos > 0 && ((*qual_out)[out_pos-1] - 33) <= 10) {
