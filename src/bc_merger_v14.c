...
void array_to_seq(const SeqArray* array, char** seq_out, char** qual_out,
                  bool* is_mixed_out, double* minor_frac_out);
...
// Sets *is_mixed_out if >= mix_pos_thresh positions have a secondary base fraction >= mix_frac_thresh.
void array_to_seq(const SeqArray* array, char** seq_out, char** qual_out,
                  bool* is_mixed_out, double* minor_frac_out) {
    int out_pos = 0;
    int n_flagged_positions = 0;
    double f2_sum = 0.0;
    ...
    *qual_out = realloc(*qual_out, out_pos + 1);
    ...
    // Report mixture status
    if (is_mixed_out)
        *is_mixed_out = (n_flagged_positions >= mix_pos_thresh);
    if (minor_frac_out)
        *minor_frac_out = (n_flagged_positions > 0) ? (f2_sum / n_flagged_positions) : 0.0;
}
