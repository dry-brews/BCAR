...
struct ProgramParams {
    ...
    size_t chunk_size;           // Max bytes per sort chunk
    size_t max_open_files;       // Max chunk files open during merge
...
class FastqReader {
    ...
    int buf_pos;  // next unread byte in buffer
    int buf_len;  // valid bytes currently in buffer

    bool fillBuffer() {
        buf_len = gzread(gz_file, buffer, sizeof(buffer));
        buf_pos = 0;
        return buf_len > 0;
    }

public:
    FastqReader(const std::string& filename) : buf_pos(0), buf_len(0) {
    ...
    bool getline(std::string& line) {
        ...
        if (is_gzipped) {
            while (true) {
                if (buf_pos >= buf_len) {
                    if (!fillBuffer()) return !line.empty();
                }
                const char* start = buffer + buf_pos;
                const char* nl = static_cast<const char*>(
                    std::memchr(start, '\n', buf_len - buf_pos));
                if (nl) {
                    line.append(start, nl - start);
                    buf_pos = static_cast<int>(nl - buffer) + 1;
                    return true;
                }
                line.append(start, buf_len - buf_pos);
                buf_pos = buf_len;  // force refill on next iteration
            }
        } else {
...
public:
        ...
        while (more_reads) {
            ...
            for (size_t i = 0; i < readers_forward.size(); ++i) {
                ...
                if (readers_forward[i]->getEntry(pair.forward)) {
                    ...
                    if (current_memory >= params.chunk_size) {
    ...
    void writeSortedChunk(std::vector<FastqPair>& chunk, size_t chunk_num) {
        ...
        std::cout << "Chunk " << chunk_num + 1 << " written ("
                  << chunk.size() << " reads)" << std::endl;
    }

    // Merge a set of sorted chunk files into a single output file pair
    void mergeChunkBatch(
            const std::vector<std::string>& fwd_files,
            const std::vector<std::string>& rev_files,
            const std::string& out_fwd,
            const std::string& out_rev) {
        FastqWriter writer_forward(out_fwd);
        std::unique_ptr<FastqWriter> writer_reverse;
        if (has_pairs) {
            writer_reverse = std::make_unique<FastqWriter>(out_rev);
        }
        ...
        for (size_t i = 0; i < fwd_files.size(); ++i) {
            auto fwd_chunk = std::make_unique<ChunkFile>(fwd_files[i]);
            if (has_pairs) {
                auto rev_chunk = std::make_unique<ChunkFile>(rev_files[i]);
                if (fwd_chunk->hasNext() && rev_chunk->hasNext()) {
                    reverse_chunks[fwd_chunk.get()] = std::move(rev_chunk);
                    pq.push(std::move(fwd_chunk));
                }
            } else if (fwd_chunk->hasNext()) {
                pq.push(std::move(fwd_chunk));
            }
        }
        ...
        while (!pq.empty()) {
            ...
            writer_forward.writeEntry(chunk->current());

            if (has_pairs) {
                auto& rev_chunk = reverse_chunks[chunk.get()];
                writer_reverse->writeEntry(rev_chunk->current());

                bool fwd_has_next = chunk->readNext();
                bool rev_has_next = rev_chunk->readNext();
                if (fwd_has_next != rev_has_next) {
                    throw std::runtime_error("Chunks out of sync: " +
                                             chunk->getFilename() + " and its reverse pair");
                }
                if (fwd_has_next) {
    ...
    // Merge sorted chunks into final output using a multi-pass merge tree.
    // Each pass reduces the number of open files by a factor of max_open_files,
    // ensuring we never exceed the OS file-handle limit while still producing
    // a globally sorted output.
    void mergeSortedChunks() {
        std::vector<std::string> working_fwd = chunk_files_forward;
        std::vector<std::string> working_rev = chunk_files_reverse;
        std::vector<std::string> intermediate_files;

        size_t pass = 0;
        size_t inter_count = 0;

        while (working_fwd.size() > params.max_open_files) {
            std::cout << "Merge pass " << pass + 1 << ": reducing "
                      << working_fwd.size() << " chunks..." << std::endl;

            std::vector<std::string> next_fwd;
            std::vector<std::string> next_rev;

            for (size_t i = 0; i < working_fwd.size(); i += params.max_open_files) {
                size_t batch_end = std::min(i + params.max_open_files, working_fwd.size());

                std::vector<std::string> batch_fwd(working_fwd.begin() + i,
                                                   working_fwd.begin() + batch_end);
                std::vector<std::string> batch_rev;
                if (has_pairs) {
                    batch_rev.assign(working_rev.begin() + i, working_rev.begin() + batch_end);
                }

                std::string out_fwd = temp_dir + "/pass_" + std::to_string(pass) +
                                      "_chunk_" + std::to_string(inter_count) + "_1.fastq";
                std::string out_rev;
                if (has_pairs) {
                    out_rev = temp_dir + "/pass_" + std::to_string(pass) +
                              "_chunk_" + std::to_string(inter_count) + "_2.fastq";
                    intermediate_files.push_back(out_rev);
                }
                intermediate_files.push_back(out_fwd);

                mergeChunkBatch(batch_fwd, batch_rev, out_fwd, out_rev);

                next_fwd.push_back(out_fwd);
                if (has_pairs) next_rev.push_back(out_rev);
                ++inter_count;
            }

            working_fwd = std::move(next_fwd);
            working_rev = std::move(next_rev);
            ++pass;
        }

        std::cout << "Final merge of " << working_fwd.size() << " chunks into output..."
                  << std::endl;
        mergeChunkBatch(working_fwd, working_rev, params.output1_file, params.output2_file);

        for (const auto& interm : intermediate_files) {
            fs::remove(interm);
        }
    }
...
ProgramParams parseCommandLine(int argc, char* argv[]) {
    ...
    if (args.find("--temp") != args.end()) {
        ...
        if (fs::space(params.temp_dir).available < params.chunk_size * 2) {  // Ensure minimum space
