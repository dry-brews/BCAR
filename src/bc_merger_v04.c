...
void print_usage(const char* program_name) {
    ...
    fprintf(stderr, "  --in1=file         Input FASTQ file 1 (required)\n");
    fprintf(stderr, "  --in2=file         Input FASTQ file 2 (optional, enables paired-end mode)\n");
...
int main(int argc, char *argv[]) {
    ...
    static struct option long_options[] = {
        {"in1", required_argument, 0, 'a'},
        {"in2", required_argument, 0, 'b'},
...
SeqArray create_seq_array(int length) {
    SeqArray arr;
    arr.length = length;
    arr.positions = calloc(length, sizeof(Position));  // Use calloc instead of malloc
    if (!arr.positions) {
...
void array_to_seq(const SeqArray* array, char** seq_out, char** qual_out) {
    ...
    *qual_out = malloc(array->length + 1);
    ...
    // Find last position with >20% of average evidence
    ...
    for(int i = 0; i <= last_good_pos; i++) {
        ...
        // Add validation
        for(int j = 0; j < 5; j++) {
            if (pos->scores[j] < -1000 || pos->scores[j] > 1000000) {
