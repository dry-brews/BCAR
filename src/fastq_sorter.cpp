...
ProgramParams parseCommandLine(int argc, char* argv[]) {
    ...
    // Required parameter
    if (args.find("--reads") == args.end()) {
        throw std::runtime_error("Required parameter --reads is missing");
    }
    
    // Split comma-separated files
    std::stringstream ss(args["--reads"]);
    std::string file;
    ...
    if (args.find("--pairs") != args.end()) {
        std::stringstream ss_pairs(args["--pairs"]);
